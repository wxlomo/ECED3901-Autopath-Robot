...
{		
	...
	//Set RPM
	left_RPM_setpoint = 60 * velocity / (tire_dia*3.14159);
	right_RPM_setpoint = 60 * velocity / (tire_dia*3.14159);			
}
...
int main(void)
{
	...
	unsigned int step_test = 0;			//Used for testing PID, 0 off, # on, OPENLOOP
	float step_RPMs[13] = {0, 20, 40, 60, 100, 150, 200, 150, 100, 60, 40, 20, 0};
	int step_index = 0;					//Used for testing PID	
	...
	{				
		...
		{			
			...
			if (temp > max_RPM) {left_RPM_setpoint= max_RPM;} 
			else if (temp< min_RPM) {left_RPM_setpoint=0;} 
			else {left_RPM_setpoint=temp;}
			printf("Enter set-point for right RPM: \n");
			scanf("%s", serial_buffer);
			temp = atof(serial_buffer);
			if (temp > max_RPM) {right_RPM_setpoint=max_RPM;} 
			else if (temp< min_RPM) {right_RPM_setpoint=0;} 
			else {right_RPM_setpoint=temp;}							
